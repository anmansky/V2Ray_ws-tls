...
#Mobile old version configuration information is suitable for version less than 1.1.0
...
is_root() {
    if [ 0 == $UID ]; then
        echo -e "${OK} ${GreenBG} The current user is root and enters the installation process ${Font}"
        sleep 3
    else
        echo -e "${Error} ${RedBG} The current user is not root. Please switch to root and re execute the script ${Font}"
        exit 1
    fi
}
judge() {
...
chrony_install() {
    ...
    chronyc tracking -v
    date
    read -rp "Please confirm whether the time is accurate and the error range ± 3 minutes (Y / N) " chrony_install
...
port_alterid_set() {
    if [[ "on" != "$old_config_status" ]]; then
        ...
        [[ -z ${port} ]] && port="443"
        read -rp "Please input alterID（default:2 Only numbers are allowed）:" alterID
        [[ -z ${alterID} ]] && alterID="2"
    fi
}
modify_path() {
...
web_camouflage() {
    ##Please note that there is a conflict between this script and the default path of LNMP script. Do not use this script in the environment where LNMP is installed, or you will be responsible for the consequences
    rm -rf /home/wwwroot
...
nginx_exist_check() {
    if [[ -f "/etc/nginx/sbin/nginx" ]]; then
        ...
        echo -e "${OK} ${GreenBG} Nginx installed by other packages is detected. Continuing to install will cause conflicts. Please install after handling${Font}"
        exit 1
    else
        nginx_install
    fi
...
nginx_install() {
    ...
    echo -e "${OK} ${GreenBG} Compilation and installation will begin soon jemalloc ${Font}"
    sleep 2
    ...
    echo -e "${OK} ${GreenBG} The compilation and installation of nginx will start soon. The process will take a long time. Please wait patiently ${Font}"
    sleep 4
    ...
    # Add configuration folder to fit old scripts
...
domain_check() {
    read -rp "Please enter your domain name information(eg:www.xraycore.net):" domain
    domain_ip=$(ping "${domain}" -c 1 | sed '1{s/[^(]*(//;s/).*//;q}')
    echo -e "${OK} ${GreenBG} Getting public IP information, please wait patiently ${Font}"
    local_ip=$(curl https://api-ipv4.ip.sb/ip)
    echo -e "DNS domain name resolution IP：${domain_ip}"
    echo -e "Local IP: ${local_ip}"
    sleep 2
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        echo -e "${OK} ${GreenBG} DNS resolution IP matches local IP ${Font}"
        sleep 2
    else
        ...
        echo -e "${Error} ${RedBG} DNS domain name resolution IP does not match the local IP. Do you want to continue the installation?（y/n）${Font}" && read -r install
        case $install in
        [yY][eE][sS] | [yY])
            echo -e "${GreenBG} continue installation ${Font}"
            sleep 2
...
port_exist_check() {
    if [[ 0 -eq $(lsof -i:"$1" | grep -i -c "listen") ]]; then
        echo -e "${OK} ${GreenBG} $1 Port not occupied ${Font}"
        sleep 1
    else
        echo -e "${Error} ${RedBG} Detected $1 Port occupied, the following is $1 Port occupancy information ${Font}"
        lsof -i:"$1"
        echo -e "${OK} ${GreenBG} After 5 seconds, it will try to kill the process automatically ${Font}"
        sleep 5
...
acme() {
    ...
    if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force --test; then
        ...
        echo -e "${Error} ${RedBG} SSL certificate test issue failed ${Font}"
        rm -rf "$HOME/.acme.sh/${domain}_ecc"
        exit 1
    fi
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/wikihacker/V2Ray_ws-tls_bash_onekey_En/${github_branch}/tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/wikihacker/V2Ray_ws-tls_bash_onekey_En/${github_branch}/http2/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
old_config_exist_check() {
    if [[ -f $v2ray_qr_config_file ]]; then
        ...
        case $ssl_delete in
        [yY][eE][sS] | [yY])
            echo -e "${OK} ${GreenBG} Old configuration retained  ${Font}"
...
enable_process_systemd() {
    ...
    if [[ "$shell_mode" != "h2" ]]; then
        ...
        judge "Setting nginx to boot automatically"
    fi
...
#    fi
#
#    judge "rc.local to configure"
#}
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/wikihacker/V2Ray_ws-tls_bash_onekey/dev/ssl_update.sh"
...
vmess_link_image_choice() {
        echo "Please select the type of link generated"
        echo "1: V2RayNG/V2RayN"
        echo "2: quantumult"
        read -rp "Please input：" link_version
...
basic_information() {
    {
        echo -e "${OK} ${GreenBG} V2ray+ws+tls Installation successful"
        ...
        echo -e "${Red} Encryption mode（security）：${Font} 自适应 "
        ...
        echo -e "${Red} Security of underlying transmission：${Font} tls "
    } >"${v2ray_info_file}"
}
show_information() {
    cat "${v2ray_info_file}"
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        ...
        echo "Please note that if you use Quantaumlt X / Router / Old edition Shadowrocket / lower than 4.18.1 Version of V2ray core Please select compatibility mode"
        echo "1: TLS1.1 TLS1.2 and TLS1.3（Compatibility mode）"
        echo "2: TLS1.2 and TLS1.3 (Compatibility mode)"
        echo "3: TLS1.3 only"
        read -rp "Please input：" tls_version
        ...
        echo -e "${Error} ${RedBG} Nginx Or the configuration file does not exist or the current installed version is h2 ，Please install the script correctly${Font}"
    fi
}
show_access_log() {
    [ -f ${v2ray_access_log} ] && tail -f ${v2ray_access_log} || echo -e "${RedBG}The log file does not exist${Font}"
}
show_error_log() {
    [ -f ${v2ray_error_log} ] && tail -f ${v2ray_error_log} || echo -e "${RedBG}The log file does not exist${Font}"
}
ssl_update_manuel() {
    [ -f ${amce_sh_file} ] && "/root/.acme.sh"/acme.sh --cron --home "/root/.acme.sh" || echo -e "${RedBG}Certificate issuing tool does not exist, please confirm whether you use your own certificate${Font}"
...
mtproxy_sh() {
    echo -e "${Error} ${RedBG} Function maintenance, not available temporarily ${Font}"
}
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/wikihacker/V2Ray_ws-tls_bash_onekey_En/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/wikihacker/V2Ray_ws-tls_bash_onekey_En/${github_branch}/install.sh
...
menu() {
    ...
    echo -e "\t---authored by WikiHacker---"
    echo -e "\thttps://github.com/Wikihacker\n"
    ...
    echo -e "${Green}11.${Font} Install 4 in 1 BBR quick installation script"
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://raw.githubusercontent.com/wikihacker/V2Ray_ws-tls_bash_onekey_En/${github_branch}/v2ray.sh)
        ;;
    4)
        ...
        ;;
    5)
        read -rp "Please input alterID:" alterID
        modify_alterid
        start_process_systemd
        ;;
    6)
